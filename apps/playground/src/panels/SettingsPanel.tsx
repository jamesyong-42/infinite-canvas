...
const inputCls = 'w-full rounded border border-neutral-200 bg-neutral-50 px-1.5 py-0.5 text-right dark:border-neutral-600 dark:bg-neutral-800 dark:text-neutral-200';
...
const btnCls = 'flex-1 rounded bg-neutral-100 py-1 text-neutral-600 hover:bg-neutral-200 dark:bg-neutral-800 dark:text-neutral-300 dark:hover:bg-neutral-700';
		...
		<div className="absolute bottom-14 left-4 z-50 w-72 max-h-[80vh] overflow-y-auto rounded-lg border border-neutral-200 bg-white/95 shadow-lg backdrop-blur-sm font-mono text-[11px] dark:border-neutral-700 dark:bg-neutral-900/95 dark:text-neutral-300">
			...
			<div className="space-y-3 p-3">
				...
				<div>
					...
					<div className="flex gap-2">
						<label className="flex flex-1 items-center gap-1">
							...
							<input type="number" step="0.01" className={inputCls} value={minZoom}
								onChange={(e) => setMinZoom(Number(e.target.value))} onBlur={applyEngine} />
						</label>
						<label className="flex flex-1 items-center gap-1">
							<span className={labelCls}>max</span>
							<input type="number" step="0.1" className={inputCls} value={maxZoom}
								onChange={(e) => setMaxZoom(Number(e.target.value))} onBlur={applyEngine} />
						</label>
					</div>
				</div>
						...
						{([
							...
							<label key={label} className="flex items-center gap-1">
								...
								<input type="number" className={inputCls} value={val}
									onChange={(e) => (set as (v: number) => void)(Number(e.target.value))} onBlur={applyEngine} />
							</label>
						))}
					</div>
				</div>
				...
				<div className={`flex gap-2 ${borderCls}`}>
					...
					<button type="button" className={btnCls}
						onClick={() => { if (engine.getNavigationDepth() > 0) { engine.exitContainer(); engine.markDirty(); } }}>
						Exit Layer
					</button>
				</div>
